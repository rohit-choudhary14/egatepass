...
<html lang="en" >
<head>
	<title>Reset Password | e-Gate Pass</title>		 	
		...
		<div class="container-fluid">
			...
			<div class="collapse navbar-collapse topnav" id="myNavbar">
				<ul class="nav navbar-nav" id="titleList">
					<li style="z-index:1;" class="list" style="z-index:1;">
						<a <?php echo "href =".$base_url; ?>>Home</a>
					</li>
				</ul>
			</div>
		</div>
	</nav>
	<div class="container text-center" id="main_div_min_height" style="min-height:400px;">
		<form class="form-horizontal" method="post">
			<div class="loading" id="loading" style="display:none;"><img src="images/loader.svg"></div>
			<h3><u id="fmHeading">Reset Password</u></h3><br />
			...
			<div class="form-group">
				...
				<div class="col-sm-5 col-md-5 col-xs-12"> 
					<div class="col-sm-7 col-md-7 col-xs-12" style="margin:0px;padding:0px;">
						<input type="password" autocomplete="off" class="form-control input-sm name_validation regExpForPassField required_validation_for_reset_pass" onblur="checkPwd();checkpassword();" id="new_password" name="new_password" placeholder="Enter new password" maxlength="100" required>
			...
			<div class="form-group">
				...
				<div class="col-sm-3">
					<input type="password" autocomplete="off" onblur="checkpassword();" class="form-control regExpForPassField input-sm name_validation required_validation_for_reset_pass" id="conf_password" name="conf_password" placeholder="Confirm password" maxlength="100" required>
			...
			<div class="form-group"> 
				<div class="col-sm-12 centered">
					...
					<button  title="Change Password" type="button" class="btn btn-primary reset_password_btn" id="reset_password" name="reset_password"><i class="fa fa-key" aria-hidden="true"></i>&nbsp;Reset Password</button>
				</div>
			</div>
		</form>
	</div>
  ...
  <script type="text/javascript">
	$(document).ready(function(e){
		...
		$(document).on('click','.reset_password_btn',function(e){
			...
			checkpassword();
			checkPwd();
			$(".required_validation_for_reset_pass").each(function(){
				var val22 = $(this).val();
				if (!val22){
			...
			if(check_required_field){ 
				...
				if(password_error_msg=='Password Mismatched'){
					...
					$('#loading').show();
					var data = 'new_pass='+btoa(new_pass)+'&user_id='+btoa(user_id)+'&csrftoken='+$('#csrftoken').val()+'&QueryType=resetPassword';
					$.ajax({
						type:"POST",
						cache:false,
						...
						success: function (data){
							...
							if(dataArr[0] == "OK"){
								...
								$('.change_text').text("Authentication failed.");
								$('#password_error_modal').modal('show');
							}
							else if(dataArr[0] == 'IVI'){
								$('.change_text').text("Invalid input values.");
								$('#password_error_modal').modal('show');
							}
							...
							else if(dataArr[0] == 'NS'){
								$('.change_text').text("Password not save please try again.");
								$('#password_error_modal').modal('show');
							}
							else{
								$('.change_text').text("User Not Register");
								$('#password_error_modal').modal('show');
							}
							$('#loading').hide();
						}
