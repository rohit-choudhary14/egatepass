<?php
	...
	$vaccineStatus = $call->SP_GET_COVID_VACCINATION_STATUS_USERS($user_id);
	...
	<form class="form-horizontal" id="vaccinationForm">
		<div style="margin-top:20px;">
			<div class="row" style="margin-bottom:15px; font-size:18px; color:#372451;">
				<label class="col-sm-12 text-center">Upload Vaccination Certificate</label>
			</div>
			...
			<div class="row">
				<div class="form-group">
					<label class="control-label col-md-4 col-sm-12">Have you taken first dose of COVID vaccine? </label>
			...
			<div class="row seconddosediv" style="<?php if($firstdose != 'Y') echo 'display: none;'; ?>">
				<div class="form-group">
					<label class="control-label col-md-4 col-sm-12">Have you taken second dose of COVID vaccine? </label>
					<div class="col-md-8 col-sm-12 text-left">
						<label class="radio-inline">
	...
	function downloadCovidPass(passid) {
		if(passid != '' && passid != null) {
			...
			ajaxRequest = $.ajax ({
				...
				data:"passid="+btoa(passid)+'&uploadfor='+btoa('S')+"&csrftoken="+$('#csrftoken').val()+"&QueryType=getCovidPassPdf",
				success:function(data) {
	...
	function enableDataCorrection(dataid) {
		if(dataid != '' && dataid != null) {
			...
			ajaxRequest = $.ajax ({
				...
				evalScripts: true,
				url:"ajax-responce.php",
				data:"dataid="+btoa(dataid)+'&uploadfor='+btoa('S')+"&csrftoken="+$('#csrftoken').val()+"&QueryType=resetVaccinationData",
				success:function(data) {
					$('#loading').hide();
					var splitData = $.trim(data).split('##');
					$('#csrftoken').val(splitData[1]);
	...
	function resendSMS(passid) {
		if(passid != '' && passid != null) {
			...
			ajaxRequest = $.ajax ({
				...
				evalScripts: true,
				url:"ajax-responce.php",
				data:"dataid="+btoa(passid)+'&uploadfor='+btoa('S')+"&csrftoken="+$('#csrftoken').val()+"&QueryType=resendVaccinationSMS",
				success:function(data) {
