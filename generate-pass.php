...
<div class="container text-center" id="main_div_min_height" style="min-height:430px;">
	...
	<?php if(isset($_SESSION['lawyer']['passtype']) && ($_SESSION['lawyer']['passtype'] == '2')){ ?>
		<div class="row text-center" style="margin-bottom:15px; margin-top:20px;">
			...
			<label>
				<input type="radio" class="passfor" name="chkpassfor" onclick="" value="L"> Litigants (for Court)
			</label>
			 &nbsp; &nbsp; &nbsp; &nbsp;
             <label>
				<input type="radio" class="passfor" name="chkpassfor" onclick="" value="LS">Litigants (for Section)
			</label> &nbsp; &nbsp; &nbsp; &nbsp;
	...
	<form class="form-horizontal" id="sectionPassFormForLitigants" name="sectionPassForm" style="display:none;">
		<div class="form-group caseDetails">
			<div class="row" style="margin-top:20px;">
				<div class="row" style="margin-top:10px;">
					<div class="col-md-12">
						<label class="control-label col-sm-2" style="text-align:left !important;">Date of Visit:</label>
						<div class="col-sm-3">
							<input type="text" autocomplete="off" class="form-control datepickercalender section_pass_dt_for_litigants" id="section_pass_dt_for_litigants" placeholder="dd/mm/yyyy" maxlength="10" onkeypress="return checkdatecharacter(event);" readonly />
						</div>
						<div class="col-sm-2"></div>
						<label class="control-label col-sm-2" style="text-align:left !important;">Purpose of Visit:</label>
						<div class="col-sm-3">
							<select name="section_pass_purpose[]" id="section_pass_purpose_forLitigants" class="section_pass_purpose_forLitigants form-control" data-placeholder="Select Purpose" multiple>
							</select>
						</div>
					</div>
				</div>
				<div class="row DIV_PURPOSE_OF_VISIT_RMK_FOR_LITIGANTS_SECTION">
				
				</div>
				<div class="row">
					
				<div class="col-md-12 DIV_PASS_FOR_LITIGANT_SECTION" style="margin-top:15px; display:none;">
					<label class="col-md-3" style="text-align:left !important"> Select Litigant Name :</label>
					<div class="col-md-3">
						 <input type="text" class="form-control uppercase" id="section_litigant_name" name="section_litigant_name" placeholder="Litigant Name" maxlength="100" style="text-transform:uppercase;" onkeypress="return AllowAlphabet(event);" autocomplete="off"/>
					</div>
					<label class="col-md-3" style="text-align:left !important">Litigant Mobile No :</label>
					<div class="col-md-3">
				...
				<div class="col-sm-12 text-center DIV_SECTION_BUTTON" style="margin-bottom:20px;">
					<a href="" style="margin-top:20px;" id="resetsectionform" class="btn btn-danger"><i class="fa fa-times"></i> Cancel</a>&nbsp; &nbsp; &nbsp; &nbsp; 
					<button type="button" style="margin-top:20px;" id="btngeneratesection" class="btn btn-primary" onclick="generatePassSectionLitigants();"><i class="fa fa-save"></i> &nbsp; Generate Pass</button>
				</div>
			</div>
		</div>
	</form>
...
<script type="text/javascript">	
	...
	$('.passfor').on('click', function(){
		...
		$('#sectionPassFormForLitigants').trigger("reset").hide();
		...
		else if($.trim($(this).val()) == 'LS'){
			$('.DIV_ALERT_MSG').hide();
			$('#courtPassForm').hide();
			$('#sectionPassFormForLitigants').show();
	...
	function generatePassSectionLitigants(){
		...
		var vist_purpose	= $.trim($('.section_pass_purpose_forLitigants').val());
		var litigant_name	= $.trim($('#section_litigant_name').val());
		var litigant_mob	= $.trim($('#section_litigant_mobileno').val());
		var mobile  = "<?php echo $mobileno; ?>";
		var advName = "<?php echo $_SESSION['lawyer']['user_name'];?>";
		...
		if(vist_purpose != ''){
			...
			for(var i = 0; i<purArr.length; i++){
				...
				purposermks.push({purpose:$.trim($("#section_pass_purpose_forLitigants option[value='"+purArr[i]+"']").text()), remark:$.trim($('#purpose_of_visit_'+purArr[i]).val())});
			}
		}
		//console.log(purposermks);
			...
			function(){
				...
				$.ajax({
					...
					success: function (data) {
						...
						$('#csrftoken').val(daraArr[1]);
						if(daraArr[0] == "OK"){
							$('#sectionPassFormForLitigants').trigger("reset");
...
<script>
	...
	function getpurposeForLitigantsSection(){
		if(purposeofvisit == 'No'){
			$('#loading').show();
			$('.close-icon-css').show();
			$('.section_pass_purpose_forLitigants').html("");
			$('.DIV_PURPOSE_OF_VISIT_RMK_FOR_LITIGANTS_SECTION').html("");
			ajaxRequest = $.ajax ({
				type: 'post',
				evalScripts: true,
				url:"ajax-responce.php",
				...
				success:function(data){
					...
					if(data[0]['status'] == 'OK'){
						...
						console.log($('.DIV_PURPOSE_OF_VISIT_RMK_FOR_LITIGANTS_SECTION').html(htmlpurposermk));
						
						$('.section_pass_purpose_forLitigants').html(htmloption);
						$('.section_pass_purpose_forLitigants').multiselect({
							enableFiltering: true,
							includeSelectAllOption: false,
							buttonWidth: '100%',
							dropRight: true
						...
						$('.section_pass_purpose_forLitigants').multiselect('refresh');
					}
					else if(data[0]['status'] == 'CSRF'){
						$('#modelText').text('Authentication Failed !!.');
						$('#blank_field_modal').modal('show');
	...
	$('.section_pass_purpose_forLitigants').on('change', function(){
		var vist_purpose = $.trim($('.section_pass_purpose_forLitigants').val());
