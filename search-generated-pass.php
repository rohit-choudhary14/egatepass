...
<html lang="en">
<head>
	<title>Previouly Generated Passes | RHC</title>		 	
	...
	<form class="form-horizontal" id="searchForm" name="searchForm" style="">
		<div class="form-group" style="margin-top:20px;">
			<div class="row" style="margin-bottom:15px; font-size:17px; color:#372451;">
				<label class="col-sm-12 text-center">Previously Generated Pass for Case Hearing</label>
			</div>
			<div class="row">
			...
			<div class="row">
				<div class="col-sm-12">
					<center>
						...
						style="margin-top:20px;" onclick="return searchenrollment();"><i class="fa fa-search"></i> &nbsp;Search</button>
					</center>
				</div>
			</div>
		</div>
	...
	function searchenrollment(){
		...
		if((fromdt != '' || todt != '') && isPastDate(todt, fromdt)){
			...
			$.ajax({
				...
				data:'fromdt='+btoa(fromdt)+'&todt='+btoa(todt)+'&passval='+btoa('C')+'&csrftoken='+csrf+'&QueryType=srchGeneratedPass',
				dataType:"JSON",
				success: function (data) {
					...
					if(data[0]['status'] == "NO"){
						...
						for(var i=1; i<data.length; i++){
							total++;
							if(isPastDate(data[i]['cl_dt'], today)){
								btnShow='';
							}
							else{
							...
							trhtml += '<tr><td style="width:5%; text-align:center !important;">'+total+'</td><td style="width:11%;">'+data[i]['type_name']+'/ '+data[i]['reg_no']+'/ '+data[i]['reg_year']+'</td><td style="width:10%;">'+data[i]['pass_no']+'</td><td style="width:26%; text-align:left !important;">'+data[i]['pet_name']+'<br/>'+data[i]['res_name']+'</td><td style="width:10%;">'+data[i]['cl_dt']+'</td><td style="width:11%;">'+data[i]['cl_type']+'</td><td style="width:10%;">'+data[i]['cl_dt']+'</td><td style="width:8%;">'+passfor+'</td><td style="width:10%;"><button type="button" onclick="sendPassAgain(this.id)" data-mob="'+mobile+'" data-id="'+data[i]['id']+'" id="btnSend_'+data[i]['id']+'" style="padding:6px 10px !important; '+btnShow+'" class="btn btn-info" data-toggle="tooltip" data-original-title="Resend ePass details"><i class="fa fa-send"></i></button><button type="button" class="btn btn-primary" onclick="showPassDetails(this.id)" data-id="'+data[i]['id']+'" data-dt="'+data[i]['cl_dt']+'" data-name="'+passname+'" id="showbtn_'+data[i]['id']+'" style="padding:6px 10px !important; margin-left:5px;" data-toggle="tooltip" data-original-title="View ePass details"><i class="fa fa-eye"></i></button></td></tr>';
						}
		...
		if(passid != '' && passid != '0'){
			...
			function(){
				...
				$.ajax({
					...
					cache:false,
					url:"ajax-responce.php",
					data:'passid='+btoa(passid)+'&passval='+btoa('C')+'&csrftoken='+$('#csrftoken').val()+'&QueryType=sendPass',
					dataType:"JSON",
					success: function (data) {
	...
	function downloadePass(passid){
		if(passid != '' && passid != null){
			...
			ajaxRequest = $.ajax ({
				...
				data:"passid="+btoa(passid)+'&download='+btoa('C')+"&csrftoken="+$('#csrftoken').val()+"&QueryType=getePassPdf",
				success:function(data){
					var dataArr = data.split('##');					
					$('#csrftoken').val(dataArr[1]);
