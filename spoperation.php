...
class spoperation
{
	...
	function SP_GENERATE_ADV_PASS($cino, $adv_type, $cldt, $cltype, $courtno, $itemno, $paddress, $party, $passfor, $partyno, $partynm, $partymob)
	{
		...
		if ($passfor == 'L' && $passtype == '2') {
			...
			if (!empty($passchk)) {
				return "PRTPASSGEN##" . $_SESSION['lawyer']['CSRF_Token'];
				exit();
			}
			...
			if (!empty($passchk)) {
				return "PPASSEXISTS##" . $_SESSION['lawyer']['CSRF_Token'];
				exit();
			}
		}
		/// CHECK PASS GENERATE OR NOT FOR LITIGANTS BY ADVOCATE
		...
		if ($passfor != 'L' && ($passtype == '1' || $passtype == '2')) {
			...
			if (!empty($passchk)) {
				return "PASSEXISTS##" . $_SESSION['lawyer']['CSRF_Token'];
				exit();
			}
		}
		// CHECK PASS GENERATE OR NOT FOR SR. ADV/ ADVOCATE FOR THIS CASE
		...
		if ($adv_type == '1' && $passfor != 'L') {
			...
			if (!empty($passchk)) {
				if (count($passchk) >= 2) {
					return "PET2PASS##" . $_SESSION['lawyer']['CSRF_Token'];
					exit();
				}
			}
		}
		...
		if ($passfor == 'L' && $passtype == '2') {
			...
			return "NA##" . $_SESSION['lawyer']['CSRF_Token'];
			exit();
		}
		...
		if ($data) {
			...
			return "OK##" . $_SESSION['lawyer']['CSRF_Token'] . '##' . $data[0]['id'];
			exit();
		} else {
			return "NA##" . $_SESSION['lawyer']['CSRF_Token'];
			exit();
