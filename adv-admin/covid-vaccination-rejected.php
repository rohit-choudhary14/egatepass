...
<div class="container text-center" id="main_div_min_height" style="min-height:430px;">
	...
	<form class="form-horizontal" id="searchForm" name="searchForm" style="">
		<div class="form-group" style="margin-top:10px;">
			<div class="row" style="margin-bottom:15px;"><h4>Rejected Vaccination Certificate Uploads</h4></div>
			<div class="row">
...
<div id="showCertificateDetails" class="modal fade" role="dialog" style="z-index:99990; margin-top:-130px !important;">
	<div class="modal-dialog modal-lg">		
		<div class="modal-content">
			<div class="modal-header">
				<span class="caption" style="font-size:15px; font-weight:bold; color:#373cd8;">View Certificate Upload</span>
				<a class="close" data-dismiss="modal">&times;</a>
			</div>
			<div class="modal-body text-justify">
					...
					<div class="col-lg-6 col-md-6 col-sm-12">
						<div class="col-sm-12 col-md-12 col-xs-12">
							<label style="margin-top:6px !important;">
								<img src="" id="imgFilepath" style="width: 100%;" />
							</label>
						</div>
					</div>
				</form>
	...
	function searchVaccinationStatus() {
		...
		if((fromdt != '' || todt != '') && isPastDate(todt, fromdt)) {
			...
			$.ajax({
				...
				data:'fromdt='+btoa(fromdt)+'&todt='+btoa(todt)+'&status='+btoa('R')+'&csrftoken='+csrf+'&QueryType=getVaccinationStatus',
				dataType:"JSON",
				success: function (data) {
					...
					if(data[0]['status'] == "NO"){
						...
						for(var i=1; i<data.length; i++){
							...
							trhtml += '<tr><td style="width:6%; text-align:center !important;">'+total+'</td><td style="width:10%;">'+data[i]['name']+'</td><td style="width:10%;">'+data[i]['regno']+'</td><td style="width:10%;">'+data[i]['mobile']+'</td><td style="width:10%; text-align:center !important;">'+data[i]['refid']+'</td><td style="width:7%;">'+data[i]['firstdose']+'</td><td style="width:7%;">'+data[i]['seconddose']+'</td><td style="width:10%;">'+data[i]['seconddosedate']+'</td><td style="width:10%;">'+data[i]['usertype']+'</td><td style="width:10%;">'+data[i]['entrydate']+'</td><td style="width:10%;"><button type="button" class="btn btn-primary" onclick="showCertificateDetails(this.id)" data-id="'+data[i]['id']+'" data-pdata="'+data[i]['name']+'@@'+data[i]['regno']+'@@'+data[i]['mobile']+'@@'+data[i]['refid']+'@@'+data[i]['seconddosedate']+'@@'+data[i]['usertype']+'@@'+data[i]['uploadfor']+'@@'+data[i]['filepath']+'@@'+data[i]['entrydate']+'@@'+data[i]['firstdose']+'@@'+data[i]['seconddose']+'" id="showbtn_'+data[i]['id']+'" style="padding:6px 10px !important; margin-left:5px;" data-toggle="tooltip" data-original-title="View Details"><i class="fa fa-eye"></i></button></td></tr>';
						}
