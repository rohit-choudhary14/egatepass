...
<div class="container text-center" id="main_div_min_height" style="min-height:430px;">
	...
	<form class="form-horizontal" id="searchForm" name="searchForm" style="">
		<div class="form-group" style="margin-top:10px;">
			<div class="row" style="margin-bottom:15px;"><h4>Approved/Rejected Registrations</h4></div>
			<div class="row">
			  ...
			  <table class="table table-striped table-bordered table-responsive" id="tbl_caselist">
				<thead>
					<tr>
						...
						<th style="width:26%; text-align:center !important;">
							Address
						...
						<th style="width:6%; text-align:center !important;">
							Action
						</th>
					</tr>
				</thead>
					...
					<div class="modal-body text-justify">
						<div class="form-group">
							<div class="col-sm-12 col-md-12 col-xs-12 text-center">
								<img src="" id="imgPartyIDLarge" style="height:450px; width:400px;">
							</div>
						</div>
					</div>
				...
				<div class="modal-body text-justify">
					<div class="form-group">
						...
						<div class="col-sm-12 col-md-12 col-xs-12">
							...
							<label class="col-md-4 lblpartyStatus" style="margin-top:6px !important;"></label>
							<label class="control-label col-md-2">
								Remark:
							</label>
							<label class="col-md-4 lblpartyRemark" style="margin-top:6px !important;"></label>
						</div>
						<div class="col-sm-12 col-md-12 col-xs-12">
							<label class="control-label col-md-2">
								Party Address:
						...
						<div class="col-sm-12 col-md-12 col-xs-12">
							...
							<label class="col-md-4" style="margin-top:6px !important;">
								...
								<br/><br/>
								<button type="button" class="btn btn-primary" data-src="" id="btnViewIdLarge" onclick="ViewIdinLarge(this.id)"> View Large</button>
							</label>
							<!--<label class="control-label col-md-2">
								Party ID:
							</label>
							<label class="col-md-4" style="margin-top:6px !important;">
								<img src="" id="imgPartyID" style="height:130px; width:100px;"><br/><br/>
								<button type="button" class="btn btn-primary" data-src="" id="btnViewIdLarge" onclick="ViewIdinLarge(this.id)"> View Large</button>
							</label>-->
						</div>
					</div>
...
<script type="text/javascript">
	function showPassDetails(id){
		...
		$('.lblpartyStatus').text(dataArr[8]);
		$('.lblpartyRemark').text(dataArr[9]);
		...
		$('#btnViewIdLarge').attr('data-src', '../StoreCandPhoto/'+dataid+'_'+$.trim(dataArr[6])+'.JPG?ab=<?php echo rand(1001, 9999); ?>');
		$('#showPassDetails').modal('show');
	}
	
	function ViewIdinLarge(btnid){
		var src = $('#'+btnid).attr('data-src');
		$('#imgPartyIDLarge').attr('src', src);
	...
	function searchpartysign(){
		...
		if((fromdt != '' || todt != '') && isPastDate(todt, fromdt)){
			...
			$.ajax({
				...
				data:'fromdt='+btoa(fromdt)+'&todt='+btoa(todt)+'&csrftoken='+$('#csrftoken').val()+'&QueryType=srchPartyinPersonAR',
				dataType:"JSON",
				success: function (data) {
					...
					if(data[0]['status'] == "NO"){
						...
						for(var i=1; i<data.length; i++){
							...
							trhtml += '<tr><td style="width:5%; text-align:center !important;">'+total+'</td><td style="width:18%;">'+data[i]['name']+'</td><td style="width:10%;">'+data[i]['contact_num']+'</td><td style="width:17%; text-align:left !important;">'+data[i]['email']+'</td><td style="width:10%;">'+data[i]['dob_str']+'</td><td style="width:26%;">'+data[i]['address']+', '+data[i]['district']+', '+data[i]['state_name']+pincode+'</td><td style="width:8%;">'+data[i]['pstatus']+'</td><td style="width:6%;"><button type="button" class="btn btn-primary" onclick="showPassDetails(this.id)" data-id="'+data[i]['id']+'" data-pdata="'+data[i]['name']+'@@'+data[i]['dob_str']+'@@'+data[i]['contact_num']+'@@'+data[i]['email']+'@@'+data[i]['idtype']+'@@'+data[i]['id_num']+'@@'+data[i]['ver_code']+'@@'+data[i]['address']+', '+data[i]['district']+', '+data[i]['state_name']+pincode+'@@'+data[i]['pstatus']+'@@'+data[i]['remark']+'" id="showbtn_'+data[i]['id']+'" style="padding:6px 10px !important; margin-left:5px;" data-toggle="tooltip" data-original-title="Approve/ Reject Login request"><i class="fa fa-eye"></i></button></td></tr>';
						}
